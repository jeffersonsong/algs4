...
public abstract class OrderedSTBaseTest extends STBaseTest {
    ...
    protected OrderedST<String, Integer> orderedST;
    ...
    protected abstract OrderedST<String, Integer> createOrderedST();
    ...
    public void testKeys() {
        insert("SEXARCHM");
        List<String> keys = toList(orderedST.keys("B", "K"));
        assertThat(keys, is(asList("C", "E", "H")));
    }

    protected void insert(String text) {
        for (int i = 0; i < text.length(); i++) {
            st.put(text.substring(i, i+1), i);
        }
    }
}