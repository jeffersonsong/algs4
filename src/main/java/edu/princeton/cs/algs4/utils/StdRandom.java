...
public final class StdRandom {
    ...
    public static int uniform(int n) {
        checkArgument(n > 0, "argument must be positive: " + n);
    ...
    public static long uniform(long n) {
        checkArgument(n > 0L, "argument must be positive: " + n);
    ...
    public static int uniform(int a, int b) {
        checkArgument((b > a) && ((long) b - a < Integer.MAX_VALUE), "invalid range: [" + a + ", " + b + ")");
    ...
    public static double uniform(double a, double b) {
        checkArgument(a < b, "invalid range: [" + a + ", " + b + ")");
        return a + uniform() * (b-a);
    }
    ...
    public static boolean bernoulli(double p) {
        checkArgument(p >= 0.0 && p <= 1.0, "probability p must be between 0.0 and 1.0: " + p);
        return uniform() < p;
    }
    ...
    public static int geometric(double p) {
        checkArgument(p >= 0, "probability p must be greater than 0: " + p);
        checkArgument(p <= 1.0, "probability p must not be larger than 1: " + p);
    ...
    public static double pareto(double alpha) {
        checkArgument(alpha > 0.0, "alpha must be positive: " + alpha);
    ...
    public static int discrete(double[] probabilities) {
        requiresNotNull(probabilities, "argument array must not be null");
        double EPSILON = 1.0E-14;
        double sum = 0.0;
        for (int i = 0; i < probabilities.length; i++) {
            checkArgument(probabilities[i] >= 0.0, "array entry " + i + " must be non-negative: " + probabilities[i]);
            sum += probabilities[i];
        }
        checkArgument(sum <= 1.0 + EPSILON && sum >= 1.0 - EPSILON,
    ...
    public static int discrete(int[] frequencies) {
        requiresNotNull(frequencies, "argument array must not be null");
        long sum = 0;
        for (int i = 0; i < frequencies.length; i++) {
            checkArgument(frequencies[i] >= 0, "array entry " + i + " must be non-negative: " + frequencies[i]);
            sum += frequencies[i];
        }
        checkArgument(sum != 0, "at least one array entry must be positive");
        checkArgument(sum < Integer.MAX_VALUE, "sum of frequencies overflows an int");
    ...
    public static int[] permutation(int n, int k) {
        checkArgument(n >= 0, "n must be non-negative: " + n);
        checkArgument(k >= 0 && k <= n, "k must be between 0 and n: " + k);
        int[] perm = new int[k];
        for (int i = 0; i < k; i++) {
    ...
    private static void validateNotNull(Object x) {
        requiresNotNull(x, "argument must not be null");
    }
